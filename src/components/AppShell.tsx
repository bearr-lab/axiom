...
export function AppShell({ children, scrollable = false }: PropsWithChildren<{ scrollable?: boolean }>) {
  return (
    <div className={`flex flex-col bg-[radial-gradient(ellipse_at_top_right,var(--tw-gradient-stops))] from-primary/20 via-background to-background text-foreground ${scrollable ? 'min-h-screen overflow-x-hidden' : 'h-screen overflow-hidden'}`}>
